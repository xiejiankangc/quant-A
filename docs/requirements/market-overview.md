...
```mermaid
flowchart LR
  S1["同花顺 CLI：指数/个股/特色/财务/估值"] --> C["采集与降级层"]
  S2["东财直连：板块/成分/个股补充信息"] --> C
...
| 板块 | 主线在哪里，是强化、回撤还是退潮？ | 板块涨跌幅、相对强度、回撤、量能趋势 | 同花顺行业+概念指数 |
| 板块内个股 | 谁是核心、谁在掉队？ | 市值分组、相对板块超额、贡献度、量能 | 指数成分 + 东财补充 |
...
| ID | 需求 | 验收标准 | 优先级 |
| --- | --- | --- | --- |
| FR-B1 | 固定板块范围为同花顺行业指数 + 概念指数（`index catalog`），不做地域板块 | 每次抓取板块代码集合稳定可对账；目录缺失时报错 | P0 |
| FR-B2 | 每日保存全部板块快照：涨跌幅、成交额、成交量 | 按 `(trade_date, thscode, source)` 幂等 upsert，重复运行不产生重复行 | P0 |
| FR-B3 | 对关注板块或全板块做历史回填 | 指定窗口内日线可重算；失败可断点续跑并记录错误 | P1 |
...
- 行情快照必须携带 `delayed` 标记；
- 主键建议使用 `(trade_date, code, source)`，用 upsert 保证重复抓取幂等；
...
| 板块体系 | 同花顺行业+概念指数为主 | 东财板块因历史接口易受 VPN 风控仅作对照 |
